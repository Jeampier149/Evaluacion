    ...
    //        $this->middleware('auth:sanctum', ['only' => ['cerrarSesion']]);
        ...
        public function register(Request $request): JsonResponse
        {
           ...
           User::create([
            ...
            return $this->sendResponse(200, 1, 'Usuario Creado exitosamente.');
        ...
        public function login(Request $request): JsonResponse
        {
            ...
            $user = Auth::user();
            $token = $user->createToken('evaluacion', ["*"], $expirationDate)->plainTextToken;
            $auth = new AuthModel();
            $menus = $auth->obtenerMenu($user->id_perfil);
            $informacion = $auth->obtenerInformacion($user->cc_empleado);
            $informacionArray = json_decode(json_encode($informacion), true);
            $servicio=$user->servicio;
            $departamento=$user->departamento;
              $userData = [
                   'username' => $user->username,
                   'nombre' => $user->nombre,
                   'cc_empleado'=> $user->cc_empleado,
                   'menu' => $menus,
                   'categoria'=>$informacionArray[0]['cc_categoria'],
                   'datos'=>$informacion,
